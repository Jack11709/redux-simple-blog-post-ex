...
Finally, import the newly created `.scss` file instead of the default Bootstrap `.css` in the beginning of your `src/root.js` file, for example:
...
following in their [`src/root.js`](src/index.js) file:
...
- If the watcher doesn’t see a file called `root.js` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebook/create-react-app/issues/1164) due to a Webpack bug.
